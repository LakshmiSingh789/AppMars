...
const List = () => {
  ...
  //       let response;
  //       if (is_superuser) {
  //         response = await axios.get(`${API_URL}/api/list/customusers/`, {
  //           headers: {
...
//   }, [items]);
  ...
  <tr>
    {columns
      .filter(col => col !== "password")  // Exclude the password column
      .map((col) => (
        <th
          key={col}
        ...
        .map((col) => (
          ...
          >
            {isImageURL(item[col]) ? (
              ...
              // Display 'None' if the value is null, undefined, or empty
              item[col] ? item[col] : 'None'
            )}
          </td>
        ))}
      <td>
        <div className="actions d-flex align-items-center">
          {isTemplate ? (
            ...
            <>
              ...
              <Link to={`/${value}/edit/${item.id}`} className="btn btn-warning btn-sm">
                <FaPencilAlt />
              </Link>
              <button
            ...
            <>
              ...
              <Link to={`/${value}/edit/${item.id}`} className="btn btn-warning btn-sm mr-2">
                <FaPencilAlt />
              </Link>
              <button
                onClick={() => handleDelete(item.id)}
