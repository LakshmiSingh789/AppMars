...
import { useLocation, Link } from 'react-router-dom'; // Import React Router's useLocation and Link components
...
const Add = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(`${API_URL}/api/insert/${value}/`,
          {
            headers: {
  ...
  const parseFormData = (formHtml) => {
    ...
    inputElements.forEach((element) => {
      ...
      if (field.type === 'date') {
        ...
        formData.initialValues[name] = field.options[0].value || '';
      } else {
  ...
  const handleFormSubmit = async (values, { setSubmitting, resetForm, setErrors }) => {
    ...
    for (const key in values) {
      if (values[key] instanceof File) {
        ...
        // Append other form data as-is
        formData.append(key, values[key]);
      }
    }

    try {
      const response = await axios.post(`${API_URL}/api/insert/${value}/`, formData, {
        headers: {
          'Authorization': `${SessionKey}`,
